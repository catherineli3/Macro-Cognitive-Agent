...
class SourceRouter:
    ...
    def _handle_cb_speech(self, raw: RawEvent) -> NormalizedEvent:
        ...
        event.importance = EventImportance.HIGH if abs(hawkish_score) > 0.5 else EventImportance.MEDIUM
    ...
    def _handle_economic_data(self, raw: RawEvent) -> NormalizedEvent:
        ...
        if "consensus" in raw.raw_data and "actual" in raw.raw_data:
            ...
            event.impact_magnitude = min(abs(event.surprise) / abs(expected) if expected else 0, 1.0)
            
            if event.surprise > 0:
                event.impact_direction = "bullish" if self._is_good_surprise(raw.headline) else "bearish"
            elif event.surprise < 0:
                event.impact_direction = "bearish" if self._is_good_surprise(raw.headline) else "bullish"
    ...
    def _handle_gov_data(self, raw: RawEvent) -> NormalizedEvent:
        ...
        if any(kw in (raw.headline + raw.content).lower() for kw in 
               ["auction", "debt ceiling", "issuance", "quarterly refunding"]):
    ...
    def _handle_institution_report(self, raw: RawEvent) -> NormalizedEvent:
        ...
        if any(kw in content_lower for kw in ["world economic outlook", "global financial stability",
    ...
    def _handle_filing(self, raw: RawEvent) -> NormalizedEvent:
        ...
        event = self._base_normalize(raw)
        event.category = "corporate_event" if raw.source == SourceType.SEC_FILING else "market_event"
    ...
    def _extract_numbers(text: str) -> dict:
        ...
        patterns = [
            (r'(CPI|PCE|PPI|GDP|NFP|unemployment|payrolls?)\s*(?::|at|of|rose|fell|increased|decreased)\s*([\d.]+%?)', 1),
    ...
    def _extract_entities(text: str) -> list[str]:
        ...
        cb_map = {
            ...
            "fomc": "FOMC", "ecb": "ECB", "european central bank": "ECB",
            "boj": "BOJ", "bank of japan": "BOJ", "pboc": "PBOC",
            "people's bank of china": "PBOC", "boe": "BOE", "bank of england": "BOE",
        }
        # Countries
        country_map = {
            "us": "United States", "united states": "United States", "america": "United States",
            "china": "China", "eurozone": "Eurozone", "europe": "Europe",
            "japan": "Japan", "uk": "United Kingdom", "britain": "United Kingdom",
    ...
    @staticmethod
    def _classify_by_keywords(text: str) -> tuple[str, Optional[str]]:
        """Classify event category and importance from keywords."""
        t = text.lower()
        
        # Category
        if any(kw in t for kw in ["fed ", "fomc", "rate hike", "rate cut", "interest rate",
                                    "monetary policy", "central bank", "basis points"]):
            category = "monetary_policy"
        elif any(kw in t for kw in ["cpi ", "inflation", "pce", "ppi", "nfp", "payroll",
                                      "gdp ", "unemployment", "jobs report", "ism "]):
            category = "economic_data"
        elif any(kw in t for kw in ["war", "conflict", "sanction", "geopolitical", "tariff"]):
            category = "geopolitical"
        elif any(kw in t for kw in ["stock", "equity", "bond", "yield", "currency", "oil ",
                                      "gold ", "market", "selloff", "rally"]):
            category = "market_event"
        else:
            category = "other"
        
        # Importance
        if any(kw in t for kw in ["breaking", "urgent", "crisis", "crash", "surprise",
                                    "unexpected", "shock", "pivot", "emergency"]):
            importance = "critical"
    ...
    def _detect_sentiment(text: str) -> str:
        ...
        bullish = sum(1 for w in ["surge", "rally", "jump", "beat", "strong", "boost",
                                    "optimistic", "easing", "dovish", "accommodative"] if w in t)
        bearish = sum(1 for w in ["plunge", "crash", "tumble", "miss", "weak", "recession",
                                    "tightening", "hawkish", "contraction", "fear"] if w in t)
    ...
    def _hawkish_dovish_score(text: str) -> float:
        ...
        hawkish_words = ["tighten", "hawkish", "restrictive", "inflation risk", "overheating",
                         "raise rates", "hike", "taper", "normalize", "exit", "withdraw"]
        dovish_words = ["ease", "dovish", "accommodative", "patient", "gradual", "data dependent",
                        "below target", "slack", "uncertainty", "downside risk", "patience"]
    ...
    def _is_good_surprise(headline: str) -> bool:
        ...
        good_high = ["nfp", "payroll", "gdp", "ism", "retail sales", "industrial production",
                     "consumer confidence", "housing starts"]
        good_low = ["unemployment", "jobless claims", "inventory"]
        h = headline.lower()
