...
from src.live_intelligence.schemas import NormalizedEvent, EventImportance
...
class EventUnderstanding:
    ...
    # Meta
    reasoned_at: str = field(default_factory=lambda: datetime.now(timezone.utc).isoformat())
...
""",
    ...
    def understand(self, event: NormalizedEvent,
                   narratives: Optional[dict] = None,
                   beliefs: Optional[dict] = None,
                   market_context: Optional[dict] = None) -> EventUnderstanding:
        ...
        understanding.is_market_moving = (
    ...
    def understand_batch(self, events: list[NormalizedEvent],
                         narratives: Optional[dict] = None,
                         beliefs: Optional[dict] = None,
                         market_context: Optional[dict] = None) -> list[EventUnderstanding]:
    ...
    def _reason_importance(self, event: NormalizedEvent) -> str:
        ...
        if event.importance == EventImportance.CRITICAL:
            reasons.append("This is a CRITICAL event that could change the macro regime or alter the dominant narrative.")
        elif event.importance == EventImportance.HIGH:
            reasons.append("HIGH importance — this provides significant new evidence for positioning decisions.")
        elif event.importance == EventImportance.MEDIUM:
            reasons.append("MEDIUM importance — adds incremental information to the existing picture.")
        
        # Surprise element
        if event.surprise is not None and abs(event.surprise) > 0:
            direction = "above" if event.surprise > 0 else "below"
            reasons.append(f"Surprise: actual came in {direction} expectations by {abs(event.surprise):.2f}, suggesting market may need to reprice.")
        ...
        return " ".join(reasons) if reasons else "Incremental information requiring further analysis."

    def _link_to_narratives(self, event: NormalizedEvent,
                            narratives: dict) -> dict:
    ...
    def _link_to_beliefs(self, event: NormalizedEvent,
                         beliefs: dict) -> list[dict]:
        ...
        if not beliefs:
            ...
            if "monetary" in event.category or "fed" in (event.title + event.summary).lower():
                results.append({
                    ...
                    "direction": "support" if "dovish" in (event.title + event.summary).lower() else "contradict",
                    "strength": 0.6,
                })
            if "inflation" in (event.title + event.summary).lower() or "cpi" in (event.title + event.summary).lower():
                results.append({
        ...
        for bid, info in beliefs.items():
            ...
            if any(w in text for w in name.split()):
                results.append({
                    "belief_id": bid,
                    "belief_name": info.get("name", bid) if isinstance(info, dict) else str(info),
                    "direction": "support" if event.impact_direction == "bullish" else "contradict",
                    "strength": min(event.impact_magnitude * 0.8, 1.0) if event.impact_magnitude else 0.5,
                })
    ...
    def _identify_unknowns(self, event: NormalizedEvent) -> list[str]:
        ...
        if event.importance in (EventImportance.CRITICAL, EventImportance.HIGH):
            unknowns.append(f"How will markets fully price this information?")
            unknowns.append(f"Will this change the central bank's reaction function?")
        ...
        if event.surprise and abs(event.surprise) > 0:
            unknowns.append(f"Is the {abs(event.surprise):.2f} surprise due to measurement error or genuine shift?")
    ...
    def _generate_questions(self, event: NormalizedEvent) -> list[str]:
        ...
        questions.extend(category_questions.get(event.category, [
    ...
    def _suggest_follow_up(self, event: NormalizedEvent) -> list[str]:
        ...
        if "nfp" in (event.title + event.summary).lower() or "payroll" in (event.title + event.summary).lower():
        ...
        return follow_ups[:3] if follow_ups else ["Related data releases", "Central bank commentary"]
    ...
    def _suggest_data_to_watch(self, event: NormalizedEvent) -> list[str]:
        ...
        category_watch = {
            ...
            "economic_data": ["Breakeven inflation", "Real yields", "Yield curve slope", "Credit spreads"],
        ...
        watch_list.extend(category_watch.get(event.category, ["VIX", "Treasury yields", "Credit spreads"]))
        return watch_list[:4]

    def _assess_confidence(self, event: NormalizedEvent,
                           understanding: EventUnderstanding) -> float:
    ...
    def _determine_depth(self, event: NormalizedEvent,
                         understanding: EventUnderstanding) -> UnderstandingDepth:
        """Determine how deep our understanding is."""
        if (understanding.beliefs_affected and understanding.assets_affected and 
            understanding.unknowns and understanding.understanding_confidence > 0.7):
    ...
    def get_understanding(self, understanding_id: str) -> Optional[EventUnderstanding]:
