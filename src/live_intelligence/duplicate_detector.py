...
from src.live_intelligence.schemas import NormalizedEvent, SourceType
...
class DuplicateDetector:
    ...
    def deduplicate(self, events: list[NormalizedEvent]) -> tuple[list[NormalizedEvent], DuplicateReport]:
        ...
        for event in events:
            ...
            if duplicate_of:
                ...
                report.duplicates.append((
                    duplicate_of.event_id, event.event_id, 
                    self._similarity(duplicate_of, event)
                ))
    ...
    def is_duplicate_of(self, new_event: NormalizedEvent, 
                        existing: NormalizedEvent) -> tuple[bool, float]:
    ...
    def _add_to_buffer(self, event: NormalizedEvent):
        ...
        content_hash = hashlib.md5(
            (event.title + event.summary).encode()
        ).hexdigest()
        self._seen_hashes.add(content_hash)

    def _find_duplicate(self, event: NormalizedEvent) -> Optional[NormalizedEvent]:
    ...
    def _within_temporal_window(self, a: NormalizedEvent, b: NormalizedEvent) -> bool:
        ...
        try:
            t_a = datetime.fromisoformat(a.occurred_at.replace('Z', '+00:00')) if a.occurred_at else datetime.fromisoformat(a.ingested_at.replace('Z', '+00:00'))
            t_b = datetime.fromisoformat(b.occurred_at.replace('Z', '+00:00')) if b.occurred_at else datetime.fromisoformat(b.ingested_at.replace('Z', '+00:00'))
        ...
        for i, e1 in enumerate(events):
            ...
            for j, e2 in enumerate(events):
                if e2.event_id in used:
                    continue
                if e1.category == e2.category and self._similarity(e1, e2) >= self.keyword_threshold:
    ...
    def _extract_keywords(text: str) -> set[str]:
        ...
        stop_words = {"this", "that", "with", "from", "have", "been", "were", "will",
                      ...
                      "after", "before", "there", "where", "what", "when"}
    ...
    def get_stats(self) -> dict:
        return {
            ...
            "duplicate_rate": (
                self.total_duplicates_found / max(self.total_processed, 1)
            ),
