...
class NewsArticle:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "source": self.source.value if isinstance(self.source, NewsSourceType) else str(self.source),
...
class ResearchEvent:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "source_type": self.source_type.value if isinstance(self.source_type, NewsSourceType) else str(self.source_type),
            "entities": self.entities,
            "country": self.country,
            "countries_affected": self.countries_affected,
            "market_impact": self.market_impact.value if isinstance(self.market_impact, ImpactDirection) else str(self.market_impact),
            "impact_severity": self.impact_severity.value if isinstance(self.impact_severity, ImpactSeverity) else str(self.impact_severity),
...
class MarketExpectation:
    ...
    actual_value: Optional[float] = None
    revision: Optional[float] = None      # Revision to prior

    # Surprise
    surprise: Optional[float] = None      # actual - consensus
    surprise_std: Optional[float] = None  # Surprise in standard deviations
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "surprise_direction": self.surprise_direction.value if isinstance(self.surprise_direction, ImpactDirection) else str(self.surprise_direction),
