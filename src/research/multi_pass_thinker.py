...
class ThinkingPassResult:
    ...
    # Delta from previous
    delta: Optional[ThinkingDelta] = None
...
class MultiPassResult:
    ...
    def total_changes(self) -> int:
        return sum(
            len(p.delta.changes) for p in self.passes if p.delta
        )
...
class MultiPassThinker:
    ...
    def execute(self, topic: str,
                ...
                belief_data: Optional[dict] = None,
                narrative_data: Optional[dict] = None,
                evidence_items: Optional[list[str]] = None) -> MultiPassResult:
    ...
    def _pass_observation(self, topic: str, macro: dict, market: dict,
                          evidence: list[str]) -> ThinkingPassResult:
...
"""
    ...
    def _pass_narrative(self, topic: str, macro: dict, market: dict,
                        obs: ThinkingPassResult,
                        narratives: dict) -> ThinkingPassResult:
        ...
        if not narratives_list:
            narratives_list = ["Data-dependent macro regime", "Policy normalization path",
                              "Growth-inflation dynamics", "Risk appetite cycle"]
...
"""
    ...
    def _pass_belief(self, topic: str, nar: ThinkingPassResult,
                     beliefs: dict, evidence: list[str]) -> ThinkingPassResult:
...
"""
    ...
    def _pass_counter(self, topic: str, bel: ThinkingPassResult,
                      beliefs: dict, evidence: list[str]) -> ThinkingPassResult:
...
"""
    ...
    def _pass_rewrite(self, topic: str, bel: ThinkingPassResult,
                      cnt: ThinkingPassResult,
                      evidence: list[str]) -> ThinkingPassResult:
...
"""
    ...
    def get_result(self, result_id: str) -> Optional[MultiPassResult]:
    ...
    def get_stats(self) -> dict:
        ...
        avg_confidence = sum(
            r.passes[-1].confidence for r in results if r.passes
        ) / max(len(results), 1)
