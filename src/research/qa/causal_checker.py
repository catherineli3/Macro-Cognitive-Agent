...
class CausalChecker:
    ...
    def verify(self, text: str) -> DimensionScore:
        ...
        mechanism_count = sum(
            ...
            for p in self.CAUSAL_MECHANISM_PATTERNS
        )
        if mechanism_count < 3:
            deductions += 30
            score.findings.append(
                "No clear causal mechanism explained — only correlations stated"
            )
        ...
        correlation_count = sum(
            len(re.findall(p, text_lower))
            for p in self.CORRELATION_ONLY_PATTERNS
        )
        ...
        confound_count = sum(
            len(re.findall(p, text_lower))
            for p in self.CONFOUNDING_PATTERNS
        )
        if confound_count == 0 and mechanism_count > 5:
            deductions += 10
            score.findings.append(
                "No acknowledgment of potential confounding factors"
            )

        # 4. Check for feedback loop consideration
        feedback_count = sum(
            len(re.findall(p, text_lower))
            for p in self.FEEDBACK_PATTERNS
        )
        if feedback_count == 0:
            deductions += 10
            score.findings.append(
                "No consideration of second-order effects or feedback loops"
            )
        ...
        causal_sentences = [s for s in sentences if any(
            re.search(p, s.lower()) for p in self.CAUSAL_MECHANISM_PATTERNS
        )]
        ...
        if score.score < 80:
            ...
            score.recommendations.append(
                "Explain the transmission channel: What → How → Why"
            )
            score.recommendations.append(
