...
class TradeChecker:
    ...
    def verify(self, text: str) -> DimensionScore:
        ...
        action_count = sum(len(re.findall(p, text_lower)) for p in self.TRADE_ACTION_SIGNALS)
        if action_count < 2:
            deductions += 30
        ...
        instrument_count = sum(len(re.findall(p, text_lower)) for p in self.INSTRUMENT_PATTERNS)
        if instrument_count < 2:
            deductions += 25
            score.findings.append("No specific instruments mentioned — trades must be expressed")

        # 3. Check risk management
        risk_count = sum(len(re.findall(p, text_lower)) for p in self.RISK_MANAGEMENT_SIGNALS)
        if risk_count == 0:
            deductions += 20
            score.findings.append("No risk management mentioned — missing stop loss, sizing")

        # 4. Check time horizon
        horizon_count = sum(len(re.findall(p, text_lower)) for p in self.HORIZON_SIGNALS)
        if horizon_count == 0:
            deductions += 10
            score.findings.append("No time horizon specified for trades")

        # 5. Check conviction level
        conviction_count = sum(len(re.findall(p, text_lower)) for p in self.CONVICTION_SIGNALS)
        if conviction_count == 0:
            deductions += 5
            score.findings.append("No conviction level stated — needed for position sizing")

        # 6. Check for vague language
        vague_count = sum(len(re.findall(p, text_lower)) for p in self.VAGUE_LANGUAGE)
        if vague_count > 3:
        ...
        if score.score < 80:
            ...
            score.recommendations.append("Add position sizing and conviction level")
            score.recommendations.append("State explicit trade horizon")
