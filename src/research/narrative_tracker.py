...
class NarrativeTracker:
    ...
    def update(
        ...
        if note:
            entry.evolution_notes.append(f"[{datetime.now().strftime('%Y-%m-%d')}] {note}")
    ...
    def get_narrative(self, narrative_id: str) -> NarrativeEntry | None:
        return self.narratives.get(narrative_id)

    def find_by_name(self, name: str) -> NarrativeEntry | None:
    ...
    def mark_broken(self, narrative_id: str, reason: str = "") -> bool:
        ...
        entry.strength = 0.0
        entry.momentum = -1.0
        entry.evolution_notes.append(f"[{datetime.now().strftime('%Y-%m-%d')}] BROKEN: {reason}")
        entry.last_updated = datetime.now(UTC).isoformat()
    ...
    def get_narrative_evolution(self, narrative_id: str) -> list[str] | None:
    ...
    def _rerank(self):
        ...
        def rank_score(n: NarrativeEntry) -> float:
            evidence_ratio = (
                n.supporting_evidence / max(n.evidence_count, 1) if n.evidence_count > 0 else 0.5
            )
