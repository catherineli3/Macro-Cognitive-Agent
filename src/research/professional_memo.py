    ...
    def _render_predictions(self) -> str:
        ...
        if self.predictions:
            ...
            for i, p in enumerate(self.predictions[:8]):
                ...
                lines.append(
                    f"| {i+1} | {pred[:80]} | {prob:.0%} | {horizon} | {invalidation[:60]} |"
                )
        else:
    ...
    def _render_trade_implications(self) -> str:
        ...
        if self.trade_implications:
            ...
            for ti in self.trade_implications[:6]:
                ...
                dir_icon = {"long": "🟢 Long", "short": "🔴 Short", "neutral": "⚪ Neutral"}.get(
                    direction, direction
                )
...
**Sources**: """
            + (
                ", ".join(self.data_sources[:5])
                if self.data_sources
                else "Automated research pipeline"
            )
            + """

**QA Grade**: """
            + (f"{self.qa_grade} ({self.qa_score:.0f}/100)" if self.qa_score else "Not graded")
        )
...
class ProfessionalMemoBuilder:
    ...
    def current_narrative(
        self, narrative: str, competing: list[str] | None = None
    ) -> ProfessionalMemoBuilder:
    ...
    def add_risk(
        self, risk: str, probability: float = 0.2, impact: str = "medium", monitoring: str = ""
    ) -> ProfessionalMemoBuilder:
    ...
    def add_invalidation(
        self, condition: str, threshold: str = "", current: str = "", implication: str = ""
    ) -> ProfessionalMemoBuilder:
