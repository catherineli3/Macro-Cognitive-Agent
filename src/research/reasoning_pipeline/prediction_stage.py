...
class PredictionStage:
    ...
    def execute(
        ...
        output.predictions = self._generate_forecasts(observation, evidence, pattern, hypothesis)

        # 2. Calibrate probabilities
        output.calibration_notes = self._calibrate_notes(hypothesis)

        # 3. Define forecast dependencies
        output.forecast_dependencies = self._define_dependencies(observation, evidence)
    ...
    def _generate_forecasts(
        ...
        policy_items = " ".join(evidence.evidence_clusters.get("monetary_policy", [])).lower()
