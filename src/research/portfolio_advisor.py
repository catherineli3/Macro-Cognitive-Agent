...
class PortfolioAdvisor:
    ...
    def recommend(self, 
                  ...
                  current_positions: Optional[dict] = None) -> PortfolioRecommendation:
        ...
        for asset_info in assets:
            asset_rec = self._recommend_asset(
                asset_info, regime, beliefs, narratives, 
                capital_flows, predictions, risks, current_positions
            )
    ...
    def get_latest(self) -> Optional[PortfolioRecommendation]:
        if self._history:
    ...
    def _recommend_asset(self, asset_info: dict, regime: str,
        ...
        if flows and asset_class in flows:
            ...
            flow_signal = str(flow_info) if not isinstance(flow_info, dict) else flow_info.get("direction", "")
    ...
    def _summarize_actions(self, 
                           recs: list[AssetRecommendation]) -> dict[str, int]:
        summary = {}
        for r in recs:
            key = r.action.value
    ...
    def _summarize_convictions(self,
                               recs: list[AssetRecommendation]) -> dict[str, int]:
        summary = {}
        for r in recs:
    ...
    def _determine_overall_stance(self, actions: dict[str, int],
                                  regime: str) -> str:
    ...
    def _identify_themes(self, recs: list[AssetRecommendation],
        ...
        for r in recs:
            ...
            elif action_type == "decrease" and r.action in (PositionAction.DECREASE, PositionAction.EXIT):
    ...
    def _sizing_guidance(self, action: PositionAction,
    ...
    def _generate_stress_scenario(self, risks: Optional[list[dict]]) -> str:
        if not risks:
    ...
    def _generate_watch_triggers(self,
                                  recs: list[AssetRecommendation]) -> list[str]:
        triggers = set()
        for r in recs[:8]:
