...
"""
    ...
    def _render_unknowns_and_watchlist(self) -> str:
        lines = ["## 7. Key Unknowns & Watchlist", "",
                 "### Top Unknowns"]
    ...
    def _render_asset_snapshot(self) -> str:
        ...
        lines.extend(["", "---", f"*Dashboard generated by Macro Research Agent v7.5. For internal use only.*"])
...
class CIODashboardBuilder:
    ...
    def regime(self, regime: str, phase: RegimePhase,
               confidence: float = 0.5,
               change_prob: float = 0.0) -> "CIODashboardBuilder":
    ...
    def add_belief(self, name: str, confidence: float = 0.5,
                   direction: str = "neutral",
                   last_changed: str = "") -> "CIODashboardBuilder":
        self._dash.top_beliefs.append({
            "name": name, "confidence": confidence,
            "direction": direction, "last_changed": last_changed,
        })
        return self

    def prediction_confidence(self, confidence: float = 0.5,
                              active: int = 0, on_track: int = 0) -> "CIODashboardBuilder":
    ...
    def add_risk(self, risk: str, probability: float = 0.1,
                 severity: str = "medium", trend: str = "—") -> "CIODashboardBuilder":
        self._dash.top_risks.append({
            "risk": risk, "probability": probability,
    ...
    def positioning(self, bias: str = "Neutral",
                    conviction: str = "Low") -> "CIODashboardBuilder":
    ...
    def add_watch_item(self, item: str, deadline: str = "",
                       importance: str = "", watch: str = "") -> "CIODashboardBuilder":
        self._dash.watch_items.append({
            "item": item, "deadline": deadline,
            "importance": importance, "what_to_watch": watch,
        })
        return self

    def add_catalyst(self, event: str, date: str = "",
                     importance: str = "", impact: str = "") -> "CIODashboardBuilder":
        self._dash.upcoming_catalysts.append({
            "event": event, "date": date,
            "importance": importance, "expected_impact": impact,
        })
        return self
    ...
    @staticmethod
    def from_research_data(topic: str, regime: str,
                           beliefs: list[dict], narratives: list[str],
                           predictions: list[dict], risks: list[dict],
                           flows: dict = None) -> CIODashboard:
        ...
        if narratives:
            builder.narrative(narratives[0] if isinstance(narratives[0], str) else str(narratives[0]))
        ...
        if risks:
            ...
            level = RiskLevel.LOW if avg_risk < 20 else RiskLevel.ELEVATED if avg_risk < 50 else RiskLevel.HIGH
