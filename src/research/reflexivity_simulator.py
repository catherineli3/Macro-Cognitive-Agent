...
class ReflexivitySimulator:
    ...
    def _if_everyone_believes(self, thesis: str, narratives: list[str] | None) -> str:
    ...
    def _if_everyone_positions(self, thesis: str, positioning: dict | None) -> str:
        if not positioning:
            return (
    ...
    def _if_crowding(self, thesis: str, positioning: dict | None) -> str:
        return (
    ...
    def _estimate_divergence(self, thesis: str, market_data: dict | None) -> float:
        if not market_data:
    ...
    def _determine_phase(self, thesis: str, divergence: float) -> ReflexivityPhase:
        if divergence < 0.2:
    ...
    def _assess_crowding(self, positioning: dict | None) -> CrowdingRisk:
        if not positioning:
    ...
    def _identify_crowded_players(self, positioning: dict | None) -> list[str]:
        if not positioning:
            return ["Institutional investors", "Hedge funds", "Retail flow"]
        return positioning.get("crowded_by", ["Multiple investor types"])

    def _summarize_positioning(self, positioning: dict | None) -> str:
        if not positioning:
    ...
    def _calculate_vulnerability(self, analysis: ReflexivityAnalysis) -> float:
        ...
        crowding_score = {
            CrowdingRisk.NONE: 0,
            CrowdingRisk.LOW: 15,
            CrowdingRisk.MODERATE: 30,
            CrowdingRisk.HIGH: 45,
    ...
    def _find_historical_analogy(self, thesis: str) -> dict:
        ...
        if any(w in thesis_lower for w in ["tech", "ai", "bubble"]):
            return {
                "period": "Dot-com bubble (1999-2000)",
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "avg_vulnerability": sum(a.vulnerability_score for a in self.analyses.values())
            / len(self.analyses),
            "extreme_crowding": sum(
                1 for a in self.analyses.values() if a.crowding_level == CrowdingRisk.EXTREME
            ),
        }
