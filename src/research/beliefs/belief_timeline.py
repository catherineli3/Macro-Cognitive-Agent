...
class BeliefTimeline:
    ...
    trough_confidence: Optional[float] = None
    peak_at: str = ""
    trough_at: str = ""
    ...
    def _recalculate(self):
        ...
        if confidences:
            ...
            self.peak_at = self.snapshots[max_idx].timestamp if max_idx < len(self.snapshots) else ""
            self.trough_at = self.snapshots[min_idx].timestamp if min_idx < len(self.snapshots) else ""
...
class BeliefEvolutionTracker:
    ...
    def record(self, belief_id: str, belief_name: str,
               confidence: float, stage: str = "",
               evidence_count: int = 0,
               supporting: int = 0, contradicting: int = 0,
               trigger_event: str = "", trigger_description: str = "",
               dominant_narrative: str = "", narrative_alignment: float = 0.0,
               domain: str = "") -> BeliefSnapshot:
    ...
    def get_timeline(self, belief_id: str) -> Optional[BeliefTimeline]:
    ...
    def get_most_volatile(self, limit: int = 5) -> list[BeliefTimeline]:
        ...
        sorted_timelines = sorted(
            ...
            key=lambda t: t.volatility, reverse=True
        )
        ...
        for s in timeline.snapshots:
            if s.trigger_event or s.trigger_description:
                events.append({
                    ...
                    "confidence_change": (
                        s.confidence - events[-1]["confidence"]
                        if events else 0
                    ),
    ...
    def get_evolution_report(self) -> str:
        ...
        for bid, timeline in self.timelines.items():
            ...
            lines.append(f"- **Confidence Range**: {timeline.confidence_range[0]:.2f} → {timeline.confidence_range[1]:.2f}")
            ...
            if timeline.peak_at:
                lines.append(f"- **Peak**: {timeline.peak_confidence:.2f} at {timeline.peak_at[:10]}")
            if timeline.trough_at:
                lines.append(f"- **Trough**: {timeline.trough_confidence:.2f} at {timeline.trough_at[:10]}")
            
            lines.append("")
        
        convergence = self.get_convergence_analysis()
        lines.append(f"## Cross-Belief Analysis")
    ...
    def get_stats(self) -> dict:
        return {
            ...
            "avg_snapshots_per_belief": (
                len(self._global_snapshots) / max(len(self.timelines), 1)
            ),
            "trends": {
                trend.value: len(self.get_beliefs_by_trend(trend))
                for trend in BeliefTrend
            },
            "convergence": self.get_convergence_analysis(),
        }
