...
class CrossAssetFlow:
    ...
    def _detect_rotations(self, summaries: list[ETFSummary]) -> list[dict]:
        ...
        bond = self._find_summary(summaries, "bonds_agg") or self._find_summary(summaries, "bonds_treasury")
        if eq and bond:
        ...
        if gold and abs(gold.weekly_flow_bn) > 1:
            rotations.append({
                ...
                "signal": (
                    "safe-haven demand" if gold.weekly_flow_bn > 0
                    else "reduced haven demand"
                ),
            })
    ...
    def _generate_narrative(
        ...
        inflows = sorted(
            ...
            key=lambda s: s.weekly_flow_bn, reverse=True,
        )
        outflows = sorted(
            [s for s in summaries if s.weekly_flow_bn < 0],
    ...
    def _find_summary(
        self, summaries: list[ETFSummary], cat: str
    ) -> Optional[ETFSummary]:
        for s in summaries:
            if s.category == cat:
                return s
