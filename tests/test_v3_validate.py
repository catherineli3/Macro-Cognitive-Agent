...
from src.schemas.learning_unit import EvidenceChange, LearningUnit, PreconditionChange
...
def test_all():
    ...
    p = Prediction(
        ...
        indicator="NASDAQ",
        direction="bearish",
        prediction_tier=PredictionTier.PRIMARY,
        transmission_channel="liquidity->equity",
        horizon="5d",
    ...
    # 2. Evaluation
    _r = EvaluationReport(batch_id="b1", directional_accuracy=0.75, accuracy_by_channel={"c1": 0.8})
    print("2. Evaluation schema OK")

    # 3. Diagnosis
    ec_correct = ErrorClassification(
        prediction_id="p1",
        is_correct=True,
        correct_category=CorrectCategory.CORRECT_STRONG,
        diagnosis_confidence=0.85,
    )
    ec_error = ErrorClassification(
        prediction_id="p2",
        is_correct=False,
        error_category=ErrorCategory.WEIGHT_ERR,
    ...
    _dr = DiagnosisReport(
        ...
        classifications=[ec_correct, ec_error],
        total_diagnosed=2,
        correct_count=1,
        incorrect_count=1,
    )
    ...
    _pc = PreconditionChange(action="add", key="cpi", value=">3%")
    _ev_change = EvidenceChange(action="add", evidence_id="ev1")
    ...
    _entry = HypothesisLibraryEntry(hypothesis_id="h1", dimension="liquidity", current_score=hs)
    print("6. HypothesisLibrary schemas OK")

    # 7. Learning Log
    lle = LearningLogEntry(
        run_id="r1", prediction_id="p1", hypothesis_id="h1", dimension="liquidity"
    )
    ...
    print("10. LearningUnitValidator OK: boundary_check passed (epsilon tolerance accounted)")
