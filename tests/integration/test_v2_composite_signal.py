...
class TestCompositeSignalGenerator:
    ...
    def test_multiple_dimensions(self):
        ...
        assert any("Liquidity" in t for t in themes)
        credit = [c for c in composites if "redit" in c.theme][0]
        assert (
            "bearish" in credit.combined_direction.value
            or credit.combined_direction == SignalDirection.BEARISH
        )
    ...
    def test_risk_off_detection(self):
        ...
        snapshot = gen.generate_snapshot(snap)
        risk_off = [
            t for t in snapshot.macro_themes if "Risk-Off" in t.name or "Risk Off" in t.name
        ]
